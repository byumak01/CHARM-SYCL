...
public:
    ...
    template <int _Dim = Dimensions, class = std::enable_if_t<_Dim == 0>>
    local_accessor_iris(handler& h, property_list const& = {});

    template <int _Dim = Dimensions, class = std::enable_if_t<(_Dim > 0)>>
    local_accessor_iris(range<Dimensions> const& size, handler& h, property_list const& = {});
    ...
    template <int _Dim = Dimensions, class = std::enable_if_t<_Dim == 0>>
    inline CHARM_SYCL_INLINE const local_accessor_iris& operator=(const value_type& rhs) const {
        *this->get_pointer() = rhs;
        return *this;
    }
...
private:
    ...
    void* ptr;
#else
    runtime::local_accessor_iris_ptr impl_;
#endif
    size_t off;
    size_t size0, size1, size2;
};

static_assert(sizeof(local_accessor_iris<int, 1>) == sizeof(void*) + sizeof(size_t) * 4);
#ifdef __SYCL_DEVICE_ONLY__
static_assert(std::is_standard_layout_v<local_accessor_iris<int, 1>>);
static_assert(std::is_trivially_copyable_v<local_accessor_iris<int, 1>>);
static_assert(std::is_trivially_destructible_v<local_accessor_iris<int, 1>>);
#endif
